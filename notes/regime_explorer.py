...
@app.cell
def _(mo, os, schema, symbol, tape):
    _handler = tape.BentoHandler(
...
@app.cell
def _(df, mo, pd, strats):
    _strategy = strats.RegimeSwitcher(switcher_config=_cfg)
...
@app.cell
def _(alt, df, mo, pd, strats):
    mo.md('### Entry Signal Timeline')

    _strategy2 = strats.RegimeSwitcher(switcher_config=_cfg)
    _rows = []
...
@app.cell
def _(df, funds, mo, strats):
    strategy = strats.RegimeSwitcher(switcher_config=_cfg)
...
@app.cell
def _(exec_result, firm_config, funds, mo, schema, strategy, symbol):
