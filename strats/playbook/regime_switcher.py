...
Implements the full decision flow: features → regime detection → weight
computation → signal combination → final positions.
...
class RegimeType(str, Enum):
    ...
    - TRENDING: directional movement, up or down
    - MEAN_REVERTING: range-bound, oscillating
    - VOLATILE: expansion, chaos, reduce exposure
    """
...
STRATEGY_KEYS = ('trend_follow', 'mean_reversion', 'volatility_breakout', 'defensive')
...
REGIME_STRATEGY_MATRIX = pd.DataFrame(
    {
        'trend_follow': [1.0, 0.2, 0.3],
        'mean_reversion': [0.1, 1.0, 0.2],
        'volatility_breakout': [0.3, 0.2, 1.0],
        'defensive': [0.0, 0.1, 0.2],
    },
    index=[
        RegimeType.TRENDING,
...
class FeatureScaler:
    ...
    Replaces sklearn dependency with a minimal frozen dataclass
    holding fitted mean and scale arrays.
    """
...
def _label_regimes(
    ...
    assigned[trend_state] = RegimeType.TRENDING
    ...
    assigned[vol_state] = RegimeType.VOLATILE
    remaining.discard(vol_state)

    # Whatever is left → MEAN_REVERTING
    for s in remaining:
        assigned[s] = RegimeType.MEAN_REVERTING
...
def detect_regime_probabilities(
    ...
    return regime_probs[  # type: ignore[return-value]
        [RegimeType.TRENDING, RegimeType.MEAN_REVERTING, RegimeType.VOLATILE]
    ]
...
def compute_strategy_weights(
    ...
    Implements the weight formula as a matrix multiply:
    ``weights = regime_probs @ matrix``, then normalizes.
...
# ---------------------------------------------------------------------------
# Performance & volatility weight adjustments
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# RegimeSwitcher — multi-strategy orchestrator
# ---------------------------------------------------------------------------
...
class RegimeSwitcher(Strategy):
    ...
    def fit(self, df: pd.DataFrame) -> RegimeSwitcher:
    ...
    def signals(self, df: pd.DataFrame) -> tuple[pd.Series, pd.Series]:  # type: ignore[type-arg]
        ...
        6. Apply performance + volatility weight adjustments
        ...
        # 6. Performance + volatility weight adjustments
