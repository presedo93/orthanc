...
def compute_aggregate(results: list[SimulationResult]) -> AggregateMetrics:
    ...
    fail_daily = sum(
        1 for r in results if r.failure_reason == FailureReason.DAILY_LOSS_BREACH
    )
    fail_total = sum(
        1 for r in results if r.failure_reason == FailureReason.TOTAL_LOSS_BREACH
    )
    fail_trailing = sum(
        1 for r in results if r.failure_reason == FailureReason.TRAILING_DD_BREACH
    )
