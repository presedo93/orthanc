"""Shared type definitions and parsing utilities for palantir module.

Technical name: types.py — timestamp parsing, resampling, and shared types.
"""
...
# OHLCV columns expected by refocus_gaze
...
def refocus_gaze(df: pd.DataFrame, timeframe: str) -> pd.DataFrame:
    """Resample an OHLCV DataFrame to a larger timeframe. Technical name: resample_ohlcv.
    ...
    Examples:
        ...
        ...     'volume': [100] * 10,
        ... }, index=idx)
        >>> resampled = refocus_gaze(df, '5min')
        >>> len(resampled)
        2
    """
    _validate_ohlcv(df)
